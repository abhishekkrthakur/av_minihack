...
model = nn_model()
model.fit(train, y_enc, nb_epoch=58, batch_size=1024, verbose=2)

preds = model.predict(test)

sample.Trip_ID = test_idx
sample.Surge_Pricing_Type = np.argmax(preds, axis=1) + 1
sample.to_csv("benchmark_nn5.csv", index=False)

pd.DataFrame(preds, columns=['a', 'b', 'c']).to_csv('nn_preds_model_5.csv', index=False)
